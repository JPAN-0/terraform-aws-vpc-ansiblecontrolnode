...
resource "aws_instance" "linux_ec2_public" {
  ...
  tags = {
    Name = "${var.namespace}-EC2-PUBLIC-${var.instance_name}"
...
resource "aws_instance" "linux_ec2_private" {
  ami                         = var.aws_ami_type
  ...
  tags = {
    Name = "${var.namespace}-EC2-PRIVATE-${var.instance_name}"
    Enviornment = "${var.enviornment_tag}"
  }
